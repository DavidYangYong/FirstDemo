package com.fl.mybatis.mapper.mapperhelper;
...
public class QueryCommonMapperHelper extends MapperHelper {
	/** 缓存skip结果 */
	 ...
	 * 默认构造方法
	 */
	public QueryCommonMapperHelper() {
	}
	 ...
	 * @param properties
	 */
	public QueryCommonMapperHelper(Properties properties) {
		this();
	...
	public void registerMapper(String mapperClass) {
		try {
			...
			throw new RuntimeException(
					"注册通用Mapper[" + mapperClass + "]失败，找不到该通用Mapper!");
		}
	}
	...
	public void setProperties(Properties properties) {
		...
		if (properties != null) {
			...
			parameterType = properties.getProperty("parameterTypes");
		}
		if (StringUtil.isNotEmpty(mapper)) {
			...
			for (int i = 0; i < parameterTypes.length; i++) {
				try {
					Class<?> class1 = Class.forName(parameterTypes[i]);
					params[i] = class1;
				} catch (ClassNotFoundException e) {
	...
	private MapperTemplate fromMapperClass(Class<?> mapperClass) {
		...
		try {
			mapperTemplate = (MapperTemplate) templateClass
					.getConstructor(Class.class, MapperHelper.class)
					.newInstance(mapperClass, this);
		} catch (Exception e) {
			throw new RuntimeException(
					"实例化MapperTemplate对象失败:" + e.getMessage());
		}
		// 注册方法
		for (String methodName : methodSet) {
			try {
				mapperTemplate.addMethodMap(methodName,
						templateClass.getMethod(methodName, parameterTypes));
	...
	// public void processConfiguration(Configuration configuration, Class<?>
	// mapperInterface) {
	// String prefix;
	// if (mapperInterface != null) {
	// prefix = mapperInterface.getCanonicalName();
	...
	// if (object instanceof MappedStatement) {
	// MappedStatement ms = (MappedStatement) object;
	// String string = StringUtils.substringAfter(ms.getId(),
	// prefix + ".");
	// boolean b = StringUtils.startsWith(string, "queryCommon");
	// if (ms.getId().startsWith(prefix)
	// && isMapperMethod(ms.getId())) {
	// if (ms.getSqlSource() instanceof ProviderSqlSource) {
	// setSqlSource(ms);
