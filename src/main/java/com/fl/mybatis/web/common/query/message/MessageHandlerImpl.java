...
import com.yunhan.scc.tools.component.module.query.QueryConstant.QueryType;
...
import com.yunhan.scc.tools.component.service.commonQuery.parser.JQGridDataParser;
...
public class MessageHandlerImpl implements MessageHandler {
	...
	public Map<String, String> buildRequestMap(HttpServletRequest request) {
		...
		for (Enumeration<?> e = request.getParameterNames(); e
			 ...
			 * value = new
			 * String(value.toString().trim().getBytes("ISO-8859-1"), "UTF-8");
			 * }
			...
			if (key.equals("page")) {
				if (null != request.getParameter("_search1")
						&& "true".equals(request.getParameter("_search1"))) {
					value = "1"; // 如果是查询或删除重置页数为1
					// } else if (null != request.getParameter("_page") &&
					// !"0".equals(request.getParameter("_page"))) {
		...
		// 处理有返回的情况
		Map<String, Map<String, Object>> backMap = (Map<String, Map<String, Object>>) request
				.getSession().getAttribute("backMap");
		if (null != backMap) {
			if (null != request.getParameter("_hisType")
				...
				if (null != thisBack) {
					Map<String, String> requestMap1 = (Map<String, String>) thisBack
							.get("postData");
					if (null != requestMap1) {
	 ...
	 */
	public void handRequestParam(Map<String, String> requestMap, QueryCondition condition)
			throws SecurityException, IllegalArgumentException,
			NoSuchFieldException, NoSuchMethodException, IllegalAccessException,
			InvocationTargetException {
		QueryType queryType = QueryType
				.getQueryType(requestMap.get(QueryConstant.QUERY_TYPE));
		switch (queryType) {
		case JQGRID:
			this.jQGridDataParser.JQGridConditionParseRule(requestMap,
					condition);
			break;
		case WEB:
		...
		case GWT:
			...
			break;
		case EXPORT:
			this.jQGridDataParser.ExportConditionParseRule(requestMap,
					condition);
			break;
		case UNKNOWN:
	 ...
	 * @throws InvocationTargetException
	 */
	public void toJson(QueryResultCount queryResultCount, StringBuffer result)
			throws JSONException, SecurityException, NoSuchMethodException,
			IllegalArgumentException, IllegalAccessException,
			InvocationTargetException {
		QueryType queryType = QueryType
				.getQueryType(queryResultCount.getCondition().getQueryType());
		switch (queryType) {
		case JQGRID:
	 ...
	 * @throws InvocationTargetException
	 */
	public void toJson(QueryResultMap queryResultCount, StringBuffer result)
			throws JSONException, SecurityException, NoSuchMethodException,
			IllegalArgumentException, IllegalAccessException,
			InvocationTargetException {
		QueryType queryType = QueryType
